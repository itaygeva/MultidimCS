...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=13,
        help="Number of sensors. Default valued is 13",
    )
...
def get_data_loaders(train_batch_size, val_batch_size,Z_dataset_train_vecs,Z_dataset_val_vecs):
    ...
    train_vecs = Z_dataset_train_vecs
    val_vecs = Z_dataset_val_vecs
...
if __name__ == "__main__":
    ...
    rows=AMBIENT_DIM; cols=N_SENSORS
    data = OD.Data(rows=rows, cols=cols, sig1vt_supp=10, k_sparse=200)
    ...
    train_loader, val_loader = get_data_loaders(BATCH_SIZE, BATCH_SIZE,Z_dataset_train_vecs,Z_dataset_val_vecs)
    ...
    checkpoint_name = f"DECONET(synthetic our data)-{ARGS.layers}L-{ARGS.amb*ARGS.nsens}-red{ARGS.red}-lr{ARGS.lr}-mu{ARGS.mu}-init{ARGS.init}.pt"
