...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=400,
        help="Artificial dimension for the synthetic signal. Default valued is 400",
    )
    ...
    parser.add_argument(
        "--red", type=int, default=4000, help="Redundancy factor. Default value =10*amb"
    )
...
#######################################################################################
def get_data_loaders(train_batch_size, val_batch_size,Z_dataset_train_vecs,Z_dataset_val_vecs):

    train = torch.from_numpy(Z_dataset_train_vecs).clone().float()
    val= torch.from_numpy(Z_dataset_val_vecs).clone().float()
    #train=Z_dataset_train_vecs
    #val=Z_dataset_val_vecs
    # train = OD.Synthetic(train_vecs)
    # val = OD.Synthetic(val_vecs)
...
def train_step(
    ...
    mse = 1000*criterion(x_pred, x_original.view(x_original.size(0), -1))/torch.norm(x_original, p='fro')
...
if __name__ == "__main__":
    ...
    # #Now: train and validate on the 'X' instance
    instance='X'
    ...
    train_loader, val_loader = get_data_loaders(BATCH_SIZE, BATCH_SIZE,X_dataset_train,X_dataset_val)
    ...
    checkpoint_name = f"DECONET(synthetic our data)-{ARGS.layers}L-{ARGS.amb*ARGS.nsens}-red{ARGS.red}-lr{ARGS.lr}-mu{ARGS.mu}-init{ARGS.init}-datasets_{instance}_{rows}x{cols}_ksparse={k_sparse}%_sig1vt_supp={sig1vt_supp}_zt_noise_sigma={zt_noise_sigma}_cs_ratio={ARGS.meas}_1000*normMseLoss.pt"
